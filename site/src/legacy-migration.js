...
export function collectLegacyData(storage = window.localStorage) {
  const notes = {};
...
function init() {
  ...
  const itemCount = (payload.state ? 1 : 0) + Object.keys(payload.notes).length + (payload.theme ? 1 : 0);
  document.querySelector("#summary").textContent =
    itemCount > 0 ? `检测到 ${itemCount} 组旧数据。` : "当前 origin 未检测到旧数据。";
  document.querySelector("#export").addEventListener("click", () => downloadJson(collectLegacyData()));
}
